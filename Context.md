...
## Audit Findings
- [PLANNED 2026-08-28] Site-Wide Response Time Audit & Performance Optimization plan (`IMPLEMENTATIONPLAN.md`) compiled for owner review. Diagnoses and ranks database N+1 query loop patterns (WS-1: implemented, resolved N+1 loop and deferred field queries), cross-ocean database latency from regional mismatches on Vercel (WS-3: under review), LocMemCache limitations in serverless environments (WS-2: under review), session backend DB request overhead (WS-2: under review), unregistered RequestTimingMiddleware (WS-2: under review), synchronous Groq API call timeouts (WS-4: implemented, resolved), and unoptimized static image assets (WS-5: implemented, resolved).
- [IMPLEMENTED 2026-08-28] Replaced the legacy checkout payment simulation flow and address form template (deleted `checkout.html`) with a direct WhatsApp order redirection. An `Order` record is still created in the database atomically for tracking/lead purposes. The target phone number is resolved dynamically from the `.env` configuration (`WHATSAPP_NUMBER`). Legacy search queries filtering solely by category (e.g. `/search/?category=OUTDOORS`) are caught in `search_view` and dynamically redirected to their respective `/view-all-products/` URLs, with category search filter parameters removed from global search. Configured Django test suite to run on SQLite in-memory automatically to bypass production Supabase pooler DB creation issues.
...
- [Open] Google Analytics 4 (G-W0ZED4768G) is blocked by Content-Security-Policy (CSP). The gtag.js script from https://www.googletagmanager.com is blocked from loading, and telemetry connections to *.google-analytics.com and *.analytics.google.com are blocked.
- [Open] Material Symbols Outlined font file size is 1,127 kB (1.1 MB). Google Fonts serves the full unsubsetted font file. We should subset it using a dynamic URL parameters text query or replace it with inline SVG icons to eliminate the request.
