...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderProducts = (products) => {
    ...
    products.forEach(product => {
      ...
      const imagePath = `images/products/${product.sku.replace(/\s+/g, '_')}.jpg`;
      
      card.innerHTML = `
        <div class="card-image-wrap">
          ${badgeHtml}
          <img src="${imagePath}" alt="${product.name}" class="product-image" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
  ...
  const openQuickView = (productId) => {
    ...
    if (modalImageContainer) {
      ...
      // We need to keep the icon for fallback, so we update the container's HTML
      modalImageContainer.innerHTML = `
        <img src="${imagePath}" alt="${product.name}" class="modal-product-image" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.style.display='none'; this.nextElementSibling.style.display='inline-block';">
        <i id="modalIcon" class="fa-solid ${iconClass} modal-icon" style="display: none;"></i>
      `;
    }
